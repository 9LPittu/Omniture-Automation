...
import com.jcrew.page.checkout.CheckoutConfirmation;
    ...
    public void get_confirmation_number() {
        if(!isProduction) {            
            if (confirmation.isOrderConfirmationPage()) {
                ...
                float total = summary.getOrderTotal();
                assertTrue("Order total is " + total + " than threshold and there is no error",
                        confirmation.hasErrors());
            }
        }
    }
