...
public class DomStepDef extends E2ECommon {
	...
	public void enterOrderNum(String filePath) throws Throwable {
		...
		testDataReader = new ExcelUtils(
				e2ePropertyReader.getProperty("windows.e2e.testdata.dir") + File.separator + filePath, "Testdata", "");
		...
		for (int i = fRow; i <= lRow; i++) {
			isExecute = String
					.valueOf(testDataReader.getCellValueFromExcel(testDataReader.getRowFromExcel(i).getCell(execute)));
			isExecuttionCompleted = String.valueOf(testDataReader
					.getCellValueFromExcel(testDataReader.getRowFromExcel(i).getCell(executionCompleted)));
			if (isExecute.equalsIgnoreCase("yes") && isExecuttionCompleted.isEmpty()) {
				orderNumber = String.valueOf(
						testDataReader.getCellValueFromExcel(testDataReader.getRowFromExcel(i).getCell(orderNum)));
	...
	public void verifyOrderStatusInSearchResulst(String status) throws Throwable {
		doStatus = DomView.verifyOrderStatus(status);
	}
	...
	public void selectOption(String option) throws Throwable {
		if (doStatus.equals("DO Created") || doStatus.equals("DO Partially Created")) {
			return;
		} else {
	...
	public void selectDistributionOrderDetails(String option) throws Throwable {
		DomView.selectOption(option);
	}
